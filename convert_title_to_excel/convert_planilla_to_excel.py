...
# Ajustamos la expresión regular:
# - (INM|AUT|CI) para la Tasa
# - Capturamos Monto y Aporte con `\$?\s*([\d\s\.,]+)` para tolerar espacios
# - Quitamos los anclajes de inicio ^ y fin $
line_pattern = re.compile(
    r'(\d+)\s+(INM|AUT|CI)\s+(\d{1,2}/\d{1,2}/\d{4})\s+(\d+/\d{4})\s+(\d+)\s+(.*?)\s+\$\s*([\d\s\.,]+)\s+\$\s*([\d\s\.,]+)'
)

def parse_line(line: str, pdf_filename: str):
    # Usamos search en lugar de match para no exigir que coincida desde el principio de la línea
    ...
    return {
        ...
        "Tasa": match.group(2),                 
        "Fecha Dda.": match.group(3),           
        "N° de Orden": match.group(4),          
        "N° Expediente": match.group(5),        
        "Actor y Demandado": match.group(6),    
        "Monto Demanda": match.group(7),       
        "Aporte": match.group(8),              
    }
